...
class Template
{
  ...
  /**
   * Array of template files that are required to successfully compile
   * this template. (The list includes this file)
   *
   * @var array
   */
   ...
   * @param Template $childTemplate A child template from which to import 
   *        sections referenced in this template.
   */
  ...
  /**
   * Override current or lazy assign (After class instantiation) the 
   * uncompiled templates path depending on your use case
   *
   * @param string $templatesPath
   ...
   * @param boolean $forceRecalc We call this function a number of times during a cycle, 
   *        so we only want to re-calculate on request
   * @param boolean $encode Change compiled filenames to MD5 encoded strings or use 
   *        the same filenames as the uncompiled templates
   * @return string
   */
   ...
   * Add underscore infront of the meta-filename to make it faster to scan the cache folder 
   * for meta and NON meta files!
   *
   ...
   * @return string
   */
  public function render($templateFilename, $data = array(), $asString = false, 
    $useCached = true, $debug = false)
  {
    ...
    try {
      ...
      {
        // NOTE: The compile() function will return a string instead of writing 
        //       directly to a file if cache = FALSE.
        // NOTE: On some shared servers, eval() might be disabled for security reasons.
        // NOTE: Don't use this option in a production setting! The intention was for 
        //       testing or running an off-line code generator.
    ...
    // Try and catch some errors here to prevent them showing in
    // your response where they can pose a security risk.
    // Not very effective! Fatal errors sail right through!!!
    // You have to register a shutdown handler to catch / stop
    ...
    catch (Exception $ex)
    {
      ...
      str_repeat('=', 60) . "\n" . get_class($ex) . 
      " Code {$ex->getCode()}: {$ex->getMessage()} in file: {$ex->getFile()} (Line {$ex->getLine()})\n" .
      str_repeat('=', 60) . "\nTrace: \n" . $ex->getTraceAsString() . "\n";
  ...
  protected function compileYield($templateString)
  {
    ...
    foreach ($yieldNames as $i => $yieldName)
    {
      ...
      // if ($childWithOverrideContent) {
        // Log::template($logPrefix . 'childWithOverrideContent->sections: ' . 
        //   print_r($childWithOverrideContent->sections, true));
      // }
      ...
      if ($childWithOverrideContent)
      {
        ...
        // If the child/override content contains a "@parent" tag, the tag will be replaced with 
        // the parent content. We work our way up the tree of parents.
        while (strpos($childContent, '@parent') !== false)
        {
          //Log::template($logPrefix . 'FOUND @PARENT directive in child content: ' . 
          //  substr($childContent, 0, 255));

          // $parentWithContent will become the first parent template to have content for this section.
          $parentWithContent = null;

          // $childWithOverrideContent will ALWAYS have a parent due to the way we set it above.
          $parent = $childWithOverrideContent->parent;
          ...
          if ( ! $parentWithContent)
          {
            ...
            // We did not find any parent content to replace the @parent tag with, so replace it with
            // an empty string.
      ...
      else
      {
        ...
        {
          // Inject yield content into section content if the section contains a yield statement 
          // coresponding to the yield content.
